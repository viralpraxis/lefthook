...
Each event must map to a hook name that is defined elsewhere in the same config. If an `ai:` mapping points to an unknown hook, `lefthook install` fails with an error listing the offending references instead of writing a command that would silently fail when the agent event fires.
...
::: callout info Tip
Commit these generated files to your repository so every team member gets the hooks automatically after cloning and running `lefthook install`.
:::
